...
const SafetySection: React.FC = () => (
  <section id="safety" className="py-4 sm:py-8 bg-gray-50">
    <div className="max-w-7xl mx-auto px-4">
      <div className="text-center mb-3 sm:mb-6">
        ...
        <h2 className="text-xl sm:text-3xl md:text-4xl font-extrabold text-gray-800">
          Cracker Safety <span className="text-red-700">Guidelines</span>
        </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-1 sm:gap-6">
        <div className="bg-white rounded-2xl border border-green-200 overflow-hidden shadow-sm">
          <div className="bg-green-600 px-3 py-2 sm:px-6 sm:py-4 flex items-center gap-2 sm:gap-3 text-white">
          ...
          <ul className="px-3 py-3 sm:px-6 sm:py-5 space-y-1.5 sm:space-y-3">
            {dos.map((tip, i) => (
              <li key={i} className="flex items-start gap-2 sm:gap-3">
                <span className="w-3.5 h-3.5 sm:w-5 sm:h-5 rounded-full bg-green-100 text-green-600 text-[9px] sm:text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5 sm:mt-1">
                  {i + 1}
                </span>
        ...
        <div className="bg-white rounded-2xl border border-red-200 overflow-hidden shadow-sm">
          <div className="bg-red-600 px-3 py-2 sm:px-6 sm:py-4 flex items-center gap-2 sm:gap-3 text-white">
          ...
          <ul className="px-3 py-3 sm:px-6 sm:py-5 space-y-1.5 sm:space-y-3">
            {donts.map((tip, i) => (
              <li key={i} className="flex items-start gap-2 sm:gap-3">
                <span className="w-3.5 h-3.5 sm:w-5 sm:h-5 rounded-full bg-red-100 text-red-600 text-[9px] sm:text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5 sm:mt-1">
                  ✕
                </span>
                <span className="text-gray-600 text-[11px] sm:text-sm leading-relaxed">{tip}</span>
              </li>
